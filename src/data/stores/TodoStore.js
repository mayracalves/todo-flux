...
import TodoConstans from "../constants/TodoConstants";
...
async function handleAction(action) {
  switch (action.actionType) {
    case TodoConstans.TODO_CREATE:
      const description = action.description;
      await createItem(description);
      TodoStore.emitChange();
      break;
    case TodoConstans.TODO_UPDATE:
      const item = action.item;
      await updateItem(item);
      TodoStore.emitChange();
      break;
    case TodoConstans.TODO_REMOVE:
      const id = action.id;
      await removeItem(id);
      TodoStore.emitChange();
      break;
    case TodoConstans.TODO_CLEAR:
      clearAll();
      TodoStore.emitChange();
      break;
    default:
